...
namespace NodeSystem.Editor.Editors.Fields
{
    public class NsObjectFieldDisplay : VisualElement
    {
            ...
            switch (evt)
            {
                ...
                default:
                {
                    if (evt.eventTypeId == EventBase<KeyDownEvent>.TypeId())
                    {
                        ...
                        {
                            if (keyDownEvent1 is not { keyCode: KeyCode.Delete } && keyDownEvent1 is not { keyCode: KeyCode.Backspace })
                                return;
                            OnKeyboardDelete();
                        }
                    }
        ...
        {
            Type objType = _objectField.ObjectType;
            ...
            return references.FirstOrDefault(reference2 => reference2 != null && objType.IsAssignableFrom(reference2.GetType()));
        }
    ...
    public class ObjectFieldSelector : VisualElement
    {
        ...
        public ObjectFieldSelector(NsObjectField objectField) => _objectField = objectField;

        [EventInterest(typeof (MouseDownEvent))]
        protected override void HandleEventBubbleUp(EventBase evt)
        {
            base.HandleEventBubbleUp(evt);
            if (evt is not MouseDownEvent { button: 0 })
                return;
            _objectField.ShowObjectSelector();
        }
    }
